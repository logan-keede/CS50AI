...
class CrosswordCreator:
    ...
    def enforce_node_consistency(self):
        ...
        for domain in self.domains:
            ...
            for val in self.domains[domain]:
                if len(val) != domain.length:
                    z.append(val)
            for val in z:
                self.domains[domain].remove(val)
    ...
    def ac3(self, arcs=None):
        ...
        while arcs != []:
            z = arcs.pop()
            if self.revise(z[0], z[1]):
                for i in self.crossword.neighbors(z[0]):
                    if (i, z[0]) not in arcs:
                        arcs.append((i, z[0]))
    ...
    def consistent(self, assignment):
        ...
        for domain in assignment:
            if (
                    len(assignment[domain]) == domain.length
                    and assignment[domain] not in al
            ):
                al.append(assignment[domain])
            else:
                return False
            ...
            for v2 in self.crossword.neighbors(v1):
                if v1 == v2:
                    ...
                    if v2 not in assignment:
                        continue
                    if v2 in assignment and assignment[v1][overlap[0]] == assignment[v2][overlap[1]]:
                        continue
                    else:
                        return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for domain in self.domains:
            if domain not in assignment:
                z.append((domain, len(self.domains[domain]), -len(self.crossword.neighbors(domain))))
